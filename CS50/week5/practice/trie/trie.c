...
bool check(char *word)
{
    // set node pointer to root of the tree
    node *cursor = root;

    // iterate over each character of the word
    for (int i = 0, n = strlen(word); i < n; i++)
    {
        // index to a partcular character
        int index = tolower(word[i]) - 'a';

        // if nothing is there, then the word doesn't exist
        if (cursor->children[index] == NULL)
        {
            return false;
        }
    ...
    // we went through every character without reaching null, so word exists
    return true;
}
